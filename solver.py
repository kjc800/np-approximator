...
def solve(list_of_locations, list_of_homes, starting_car_location, adjacency_matrix, params=[]):
    ...
    homes = [list_of_locations.index(home) for home in list_of_homes]
    start = list_of_locations.index(starting_car_location)
    dropoffs = {s: [s] for s in homes}
    cycle = repeatedDijkstras(g, start, homes)
    

    return (cycle, dropoffs)
...
def repeatedDijkstras(graph, start, homes):
    ...
    dijkstra = minPath(graph, start, homes_cpy)
    del dijkstra[1][-1]
    cycle.extend(dijkstra[1])
    lastEnd = dijkstra[0]
    homes_cpy.remove(dijkstra[0])

    while len(homes_cpy) > 0:
        dijkstra = minPath(graph, lastEnd, homes_cpy)
        del dijkstra[1][-1]
        cycle.extend(dijkstra[1])
        lastEnd = dijkstra[0]
        homes_cpy.remove(dijkstra[0])
